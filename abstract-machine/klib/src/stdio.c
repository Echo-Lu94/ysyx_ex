...
// 则在 str 中首先填放剩余宽度值指出的空格数
    ...
    if (type & SPECIAL){
        if (base==8)// 对于八进制转换结果头一位放置一个'0'
            *str++ = '0';
        else if (base==16) {// 对于十六进制则存放'0x'
            *str++ = '0';
            *str++ = digits[33];
        }
    }
...
//            if(!s) s = "NULL";
            ...
            while (len < field_width--)
                *str++ = ' ';
            continue;//用break只会跳出switch,continue表示跳出for循环
...
//            break;
    ...
    if(qualifier == 'l'){
        ...
        if(flags & SIGN) num=va_arg(args, int);//short
        else             num=va_arg(args, unsigned int);//unsigned short
    }else if(flags & SIGN){
        num=va_arg(args, int);
    }else{
