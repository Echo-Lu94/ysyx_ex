...
//  int bits2=BITS(i,30,21)<<1;
  switch (type) {
    ...
    case TYPE_R: src1R(); src2R();         
                 ...
                 break;//rd
    case TYPE_N:if(i == 0x00100073 )       break;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, t = s->pc + 4; s->pc = (src1 + imm) & (0xfffffffe); R(rd) = t);//ret
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->pc += imm);//j
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2) s->pc += imm);//bnez
//add-longlong
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (word_t)src1 < (word_t)src2);//snez
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);
//bit
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2) s->pc +=imm);//beqz 
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = src1 >> imm );//TODO: sext//默认有符号数右移补1
...
//bubble-sort
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if(src1 >= src2) s->pc += imm);//blez
//crc32
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (word_t)src1 >> imm);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if((word_t)src1 >= (word_t)src2) s->pc += imm);
//div
...
//switch
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if((word_t)src1 < (word_t)src2) s->pc += imm);
